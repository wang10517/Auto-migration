## This file contains a list of xpath selecotrs needed by this module 
## to achieve its functionality. This file needs to be set per domain 

# Following constants appear in controller.py
PROCESSDOMAIN = 9
STARTSITEID = 0
PATHTODOMAINSOURCE = "Sites\Organizing Complex P2S migrations.xlsx"
PATHTOEXTENSION = '/Users/Shengkai/Downloads/PROWL-Toolbar_v2.13.1 (1).crx'

# PRODUCTNAME_CLASSNAME = 'page_headers'
PRODUCTNAME_XPATH = '//h1[@class = "vp-product-title"]//span'
ALL_SELECT_XPATH = '//div[@class = "opt-field"]'
CLICKBAR_XPATH_t1 = '//div[@class = "dropdown-format"]'
CLICKBAR_XPATH_t2 = '//div[@class = "radio-format"]'
CLICKBAR_XPATH_t3 = '//div[@class = "checkbox-format"]'
# CLICKBAR_CLASSNAME = 'buybox-dropdown-selector'
# SELECT_ATTR_CSS = 'div.product-variant-selector p'
SELECT_ATTR_XPATH = '//span[@class = "label"]'

OPTIONS_XPATH_t1 = ".//option" #(select from the second one)
OPTIONS_XPATH_t2 = ".//div[@class = 'radio-option']"
OPTIONS_XPATH_t3 = "./div"
